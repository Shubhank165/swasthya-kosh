...
from questioning_agent.core.enums import Certainty
...
from questioning_agent.interpretation.free_text_parser import parse_free_text
from questioning_agent.interpretation.numeric_parser import parse_numeric
from questioning_agent.interpretation.option_parser import (
    parse_multi_select,
    parse_single_select,
)
...
def _slots():  # type: ignore[no-untyped-def]
    ...
    # parents[3] here, parents[2] in conftest: one of them had to be wrong, and it was this
    # one. Take the content root from conftest so there is a single definition of it.
    from tests.questioning.conftest import CONTENT

    assert Path is not None  # keep the import meaningful for readers
