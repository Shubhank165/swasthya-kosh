...
#:
#: **Keyed by the ids this kiosk actually emits**, which are the bare ones in
#: `clinical/questions.py` (`ask_age`, `ask_severity`) and `registration.<key>` —
#: *not* the dotted ids of the phone app's compiled bundle. The two live side by
#: side in this repo and only one of them reaches `flow.screen`; a table keyed
#: by the other compiles, ships and silently never fires.
#:
#: **This is not the general mechanism for choosing an input.** `ANSWER_UI` in
#: `clinical/questions.py` already is, and it already routes `ask_severity` to
#: Wong-Baker faces and `ask_age` to age bands — both better for a low-literacy
#: patient than a row of digits. This table covers only the gap those leave:
#: registration, which `answer_ui` is never consulted for and which currently
#: offers the tablet no touch input at all, so an elderly patient who will not
#: speak cannot answer it.
#:
#: Anything absent keeps the input it has today. Before adding an entry, check
#: whether `ANSWER_UI` should carry it instead.
