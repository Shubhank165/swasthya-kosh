...
| `queue/` | Full OPD queue domain: tickets, instances, priority classes, recall, no-show, overtake accounting | The spec does not ask for queue management. `app/domain/worklist.py` keeps the one part that is needed — arrival ordering — and this is the rest. |
